...
namespace Windows.Media.Playback
{
	public partial class MediaPlayer : NSObject
	{
		...
		private void OnMediaEnded(object sender, NSNotificationEventArgs args)
		{
			...
			if (Source is MediaPlaybackList)
			{
				var asset = ((AVPlayerItem)args.Notification.Object).Asset as AVUrlAsset;
				var itemIsLastInQueue = asset.Url.AbsoluteString == ((MediaPlaybackList)Source).Items.Last().Source.Uri.AbsoluteUri;
				if (!itemIsLastInQueue)
				{
					return;
				}
			}
